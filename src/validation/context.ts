...
export class ValidationContext {
  ...
  error(t: ts.Type, error?: [string?, string?]): ts.Statement {
    if (this.resultType.return)
      return factory.createReturnStatement(this.resultType.return);
        ...
        ? genStr(
            error?.[0] ||
              "Expected " +
                errPath +
                (error?.[1] ||
                  ` to be ${this.transformer.checker.typeToString(t)}.`)
          )
        : genAdd(
            genAdd(genStr(error?.[0] || "Expected "), errPath),
            genStr(
              error?.[1] ||
                ` to be ${this.transformer.checker.typeToString(t)}.`
            )
          );
    if (this.resultType.returnErr)
      return factory.createReturnStatement(errMessage);
